    ...
    static bool Prefix(ref IEnumerable<DynamicVar> __result)
    {
        if (RevertAnthony.IsVersion("memento-mori", "v0.99.1"))
        {
            __result = new DynamicVar[]
            {
                ...
                new CalculatedDamageVar(ValueProp.Move).WithMultiplier((CardModel card, Creature _) => CombatManager.Instance.History.Entries.OfType<CardDiscardedEntry>().Count((CardDiscardedEntry e) => e.HappenedThisTurn(Compat.GetCombatState(card)) && e.Card.Owner == card.Owner)),
            };
            return false;
        }
        return true;
