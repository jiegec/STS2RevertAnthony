...
static class Spite_ShouldGlowGoldInternal_Patch
{
    static bool Prefix(ref bool __result)
    {
        if (RevertAnthony.IsVersion("spite", "v0.99.1"))
        {
            ...
            __result = CombatManager.Instance.History.Entries.OfType<DamageReceivedEntry>().Any((DamageReceivedEntry e) => e.Receiver != null && e.HappenedThisTurn(Compat.GetCombatState(e.Receiver)) && e.Result.UnblockedDamage > 0 && e.GetCurrentSide() == CombatSide.Player);
            return false;
        }
        return true;
    }
...
static class Spite_OnPlay_Patch
{
    ...
    static async Task OldOnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay, Spite instance)
    {
        ...
        bool tookDamageThisTurn = CombatManager.Instance.History.Entries.OfType<DamageReceivedEntry>().Any((DamageReceivedEntry e) => e.HappenedThisTurn(Compat.GetCombatState(instance)) && e.Receiver == instance.Owner.Creature && e.Result.UnblockedDamage > 0 && e.GetCurrentSide() == CombatSide.Player);
